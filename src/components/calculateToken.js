...
function calculateToken(_stage, _crypto, _amount) {
  ...
  if (_crypto === Null) {
    ...
    _tokens = tokenPrices[_wbnbAddress] * _amount * (10000 / _rate);
  } else if (_crypto === _usdtAddress) {
    _tokens = tokenPrices[_usdtAddress] * _amount * (10000 / _rate);
  } else if (_crypto === _usdcAddress) {
    _tokens = tokenPrices[_usdcAddress] * _amount * (10000 / _rate);
  }
  return _tokens;
}
